...
class WeatherClient(object):
    ...
    def almanac(self, location):
        ...
        return_response["high"]["normal"] = \
            xml.response.almanac.temp_high.normal.C.cdata
        return_response["high"]["record"] = \
            xml.response.almanac.temp_high.record.C.cdata
        return_response["high"]["year"] = \
            xml.response.almanac.temp_high.recordyear.cdata

        return_response["low"] = {}
        return_response["low"]["normal"] = \
            xml.response.almanac.temp_low.normal.C.cdata
        return_response["low"]["record"] = \
            xml.response.almanac.temp_low.record.C.cdata
        return_response["low"]["year"] = \
            xml.response.almanac.temp_low.recordyear.cdata
